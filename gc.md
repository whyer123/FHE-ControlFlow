...
5. 另建 v2 integer LWE 路線，直接使用 OpenFHE 產生的 `a'`、`b'`、`one'` 和 `hsk.s_mod_q` 建立 `GC{[Dec_hsk(x') <= Dec_hsk(b')]}` runtime demo。
...
以及 demo 用的 rounding/decode shape。早期 `ControlledRevealCircuit` path 還是 mock/fixed material；v2 integer LWE path 已經把 OpenFHE 匯出的真實 ciphertext fields 接進 Boolean circuit input。
...
目前測試會先執行 `export_openfhe_lwe_int_material` 動態產生臨時 material 檔，再由 `OpenFHELWEIntMaterial` parser 讀回 `a'=Enc(3)`、`b'=Enc(7)`、`hsk.s_mod_q`，驗證 plain Boolean circuit evaluation：
...
9. v2 OpenFHE integer decrypt-compare circuit 已接到 garbled artifact/evaluation API。`test_openfhe_lwe_int_decrypt_compare_gc.sh` 目前會先產生新的 OpenFHE material，再在本機使用 in-repo minimal GC backend 驗證：
...
13. in-repo minimal GC 仍保留為無 EMP 環境的 fallback，不是主要 demo path。
14. `openfhe_controlled_reveal_reference.cpp` 已移除 semantic gate decode / fake re-encrypt，改成 `BootstrapGateCoreOpenFHE -> EvalAccCGGI -> SwitchCTtoqn` 的資料流；但 `ExternalProductCGGI`、blind rotation key material、key switching key material 還不是 OpenFHE 1.5.0 bit-accurate 展開。
